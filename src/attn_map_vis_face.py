...
try:
    age = AGE_GENDER_RACE_MAP["age"][attr[img_no]["age"]]
    gender = AGE_GENDER_RACE_MAP["gender"][id_attr[img_id]["gender"]]
    race = AGE_GENDER_RACE_MAP["race"][id_attr[img_id]["race"]]

    edit_age = AGE_GENDER_RACE_MAP["age"][3] # thirties
    edit_gender = AGE_GENDER_RACE_MAP["gender"][1] # woman
    edit_race = AGE_GENDER_RACE_MAP["race"][2] # asians

    original_prompt = generate_prompt(gender)
    print("prompt: ", original_prompt)
    edit_prompt = generate_prompt(edit_gender)
    print("edit_prompt: ", edit_prompt)
except:
    original_prompt = "The face of a woman"
    edit_prompt = "The face of a woman"
...
pipe.scheduler.set_timesteps(20, device)

controller = AttentionStore(res=res)

vae_latent = image2latent(pipe, Image.open(img_path).resize((res, res)))

print("inversion...")
diff_latent = accelerated_invert(
    model=pipe,
    start_latents=vae_latent,
    prompt=original_prompt,
    guidance_scale=guidance_scale,
    num_inference_steps=num_inference_steps,
    start_step=start_step,
    num_fixed_point_iters=6
)
...
try:
    im_save_dir = f"../logs/attn_map_vis/{int(img_id):04d}_{int(img_no):05d}_{original_prompt.replace(' ', '_')}"
except:
    im_save_dir = f"../logs/attn_map_vis/target_{original_prompt.replace(' ', '_')}"
os.makedirs(im_save_dir, exist_ok=True)
resampled_img.save(os.path.join(im_save_dir, f"resampled_face_ss={start_step}.png"))
