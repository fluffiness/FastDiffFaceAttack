...
if __name__ == '__main__':
    ...
    parser.add_argument('--gen_img_dir', default="../logs/amt-gan_asset_backup/generated_imgs_jpg/facenet", type=str)
    ...
    asc = 0
    for ref in refs:
        ref_dir = os.path.join(args.gen_img_dir, ref)
        print(f"In {ref}")
        for img_file in tqdm(os.listdir(ref_dir), bar_format='{l_bar}{bar:50}{r_bar}{bar:-10b}'):
            img_path = os.path.join(ref_dir, img_file)
            img = Image.open(img_path).resize((res, res))
