...
class DatetimeLogger:
    ...
    def __init__(self, log_dir, sub_name=None):
        current_datetime = datetime.now()
        formatted_datetime_string = current_datetime.strftime("%m-%d_%H:%M:%S")
        if sub_name:
            self.log_dir = os.path.join(log_dir, f"{formatted_datetime_string}_{sub_name}")
        else:
