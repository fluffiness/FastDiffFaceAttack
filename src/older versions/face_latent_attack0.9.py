...
def targeted_cross_attention_loss_new(
    controller: StructureLossAttentionStore, 
    batch_size: int,
    target_prompt: str,
    target_att_map: torch.Tensor,
    ...
    if config.training.w_blur:
        P = Fv.gaussian_blur(P.permute((0, 3, 1, 2)), kernel_size=config.training.ca_gaussian_kernel_size, sigma=1.0)
        P = P.permute((0, 2, 3, 1))
        Q = Fv.gaussian_blur(Q.permute((0, 3, 1, 2)), kernel_size=config.training.ca_gaussian_kernel_size, sigma=1.0)
        Q = Q.permute((0, 2, 3, 1))
...
def identity_attack_memory_efficient( # for age_gender_race prompts
    ...
    lam_semseg = config.training.semseg_lam
    lam_pair = config.training.pairwise_loss_weight
    lam_neighbor = config.training.neighborhood_loss_weight
    ...
    for e in range(n_epoch):
        ...
        for it, batch in enumerate(train_loader):
            ...
            # # check resampling result
            # print(model.scheduler.timesteps)
            # clean_vae_latents = intermediate_latents[-1][:batch_size]
            # clean_adv_images = latent2image(model, clean_vae_latents)
            # clean_adv_images = Image.fromarray(clean_adv_images[0])
            # clean_adv_images.save("clean_adv_images.jpg")
            # sample_adv_images = latent2image(model, adv_vae_latents)
            # sample_adv_images = Image.fromarray(sample_adv_images[0])
            # sample_adv_images.save("sample_adv_imgs.jpg")
            # exit()
            ...
            if lam_pair > 0.0:
                ...
                for i in range(batch_size):
                    for j in range(i + 1, batch_size):
                        ...
                        pairwise_loss += lam_pair * cos_dist_loss(adv_embeddings_i, adv_embeddings_j) / num_pairs
            ...
            if lam_neighbor > 0.0:
                neighbor_loss += lam_neighbor * neighborhood_struct_loss(adv_embeddings, target_embedding)
            ...
            if lam_semseg > 0.0:
                ...
                parse_loss = lam_semseg * parse_loss_fn(parse_map_adv, parse_maps_labels)
                total_loss += parse_loss
            ...
            for i in reversed(list(range(start_step, num_inference_steps))):
                ...
                # 4-1. calculate self-attention loss
                ...
                step_sa_loss = lam_sa * controller.loss
                step_loss_for_grad = torch.sum(next_latents[batch_size:] * grad) + step_sa_loss # next_grad = d/dx(f_r) * grad + d/dx(step_loss)
                ...
                controller.loss = 0.0
                controller.reset()
                del noise_pred_from_pre_calc, next_latents, step_sa_loss, step_loss_for_grad
                ...
                if lam_ca > 0.0:
                    ...
                    if not ca_loss_is_targeted:
                        ...
                        step_ca_loss = targeted_cross_attention_loss_new(
            ...
            logger.log((
                ...
                f"parse_loss: {parse_loss.item():.3f}, "
